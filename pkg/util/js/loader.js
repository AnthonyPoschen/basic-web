...
const collectUndefinedElements = () => {
  ...
  const walk = (root) => {
    ...
    root
      .querySelectorAll("*")
      .forEach((el) => el.shadowRoot && walk(el.shadowRoot));
  };
...
const appendElementTemplates = (html) => {
  ...
  div
    ...
    .forEach((template) => document.head.appendChild(template));
};
  ...
  div.querySelectorAll("script").forEach((s) => {
    ...
    if (ns.type === "module") {
      ...
      elementNames.forEach((name) =>
        moduleDefinitions.push(customElements.whenDefined(name)),
      );
    } else if (s.src) {
      moduleDefinitions.push(
        new Promise((resolve, reject) => {
          ...
          ns.addEventListener(
            ...
            () =>
              reject(
                new Error(`failed to execute element script: ${elementUrl}`),
              ),
            { once: true },
          );
        }),
...
const installElementSource = (source) => {
  if (!source.install) {
    source.install = (async () => {
      ...
      await appendElementScripts(source.html, source.elementUrl, [
        ...source.names,
      ]);
    })();
  }
...
const hydrate = async (root) => {
  ...
  try {
    if (!customElements.get(name)) await loadElementTree([name]);
    await requestScan();
  } catch (error) {
    console.error(error);
  } finally {
    ...
    root.dispatchEvent(
      new CustomEvent("basic-web:ready", { bubbles: true, composed: true }),
    );
  }
};
...
const requestScan = () => {
  ...
  if (!activeScanPromise) {
    activeScanPromise = runScanLoop().finally(() => {
...
if (window.location.hostname === "localhost") {
  // Hot reloading
  new EventSource("/dev/reload").onmessage = () => {
